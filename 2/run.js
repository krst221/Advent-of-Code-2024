...
function isSequenceSafe(sequence) {
  ...
  const ascending = sequence[0] <= sequence[1]
  ...
  for (let i = 0; i < sequence.length - 1; i++) {
    ...
    if (ascending) {
      if (next <= current || next - current < 1 || next - current > 3) {
        return false
      }
    } else {
      if (next >= current || current - next < 1 || current - next > 3) {
        return false
      }
    }
  }
...
let safeReports = 0
let potentiallySafeReports = 0

for (const item of data) {
  if (isSequenceSafe(item)) {
    safeReports++
  } else {
    // Try removing one number to see if the item can be safe
    let canBeSafe = false

    for (let j = 0; j < item.length; j++) {
      const temp = item.slice(0, j).concat(item.slice(j + 1))
      if (isSequenceSafe(temp)) {
        canBeSafe = true
        break
      }
    }
...
console.log(
  "Potentially safe reports by removing one number:",
  potentiallySafeReports + safeReports
)
